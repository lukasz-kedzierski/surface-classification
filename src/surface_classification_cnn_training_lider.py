...
INPUT_SIZE = 8
NUM_EPOCHS = 100
DATA_DIR = Path('../data/testypodloz/csv/')
LOOKBACK = 2.0
SAMPLING_FREQUENCY = 100.
DATASET_FREQUENCY = 100.
SUBSET = ('imu', 'servo')

with open('../data/testypodloz/labels.json') as fp:
    labels = json.load(fp)

dataset = [(DATA_DIR.joinpath(key + '.csv'), values['surface']) for key, values in labels.items()]
...
for epoch in range(NUM_EPOCHS):
    ...
    if early_stopper.counter == 0:
        best_model = copy.deepcopy(cnn_model.state_dict())

model_name = 'cnn_classifier_' + '_'.join((str(num_classes),) + SUBSET) + '_' + time.strftime(
    "%Y-%m-%d-%H-%M-%S")
